...
double HTValence ( double xb, double xi, double t, double Qsq, double bu, int n ) {
    ...
    double deltaVal = 0.33;
    double k = deltaVal + alphastr * t - 0.5;
...
//TODO twist2 - not cross checked until now!
...
amplitude getAmplitude(double _qsq,double _xi,double _xbj, double _t)
{
    ...
    EBarxi = 1./sqrt(2.0)*(charge_u*EBarU(_xi,_xi,_t,_qsq,ebu)-charge_d*EBarD(_xi,_xi,_t,_qsq,ebd));
    HTxi =  1./sqrt(2.0)*(charge_u*HTValence(_xi,_xi,_t,_qsq,hbu,1)-charge_d*HTValence(_xi,_xi,_t,_qsq,hbd,2));
    ...
    for (unsigned int i = 0; i < 128; i ++) {
        ...
        double etildelow = 1./sqrt(2.0)*(charge_u*ETilde(xLow[i],_xi,_t,_qsq,etu,0)-charge_d*ETilde(xLow[i],_xi,_t,_qsq,etd,1));
        double etildehigh = 1./sqrt(2.0)*(charge_u*ETilde(xHigh[i],_xi,_t,_qsq,etu,0)-charge_d*ETilde(xHigh[i],_xi,_t,_qsq,etd,1));
        ...
        ebarh = 1./sqrt(2.0)*(charge_u*EBarU(xHigh[i],_xi,_t,_qsq,ebu)-charge_d*EBarD(xHigh[i],_xi,_t,_qsq,ebd));
        ebarl = 1./sqrt(2.0)*(charge_u*EBarU(xLow[i],_xi,_t,_qsq,ebu)-charge_d*EBarD(xLow[i],_xi,_t,_qsq,ebd));
        HTh =  1./sqrt(2.0)*(charge_u*HTValence(xHigh[i],_xi,_t,_qsq,hbu,1)-charge_d*HTValence(xHigh[i],_xi,_t,_qsq,hbd,2));
        HTl =  1./sqrt(2.0)*(charge_u*HTValence(xLow[i],_xi,_t,_qsq,hbu,1)-charge_d*HTValence(xLow[i],_xi,_t,_qsq,hbd,2));
