...
double HVector3::Angle(const HVector3 & q) const
{
...
HVector3 HVector3::Unit() const
{
   double tot2 = Mag2();
   double tot = (tot2 > 0) ?  1.0/sqrt(tot2) : 1.0;
   HVector3 p(x*tot,y*tot,z*tot);
   return p;
}
////////////////////////////////////////////////////////////////////////////////
/// Rotate vector around X.

void HVector3::RotateX(double angle) {
   double s = sin(angle);
   double c = cos(angle);
   double yy = y;
   y = c*yy - s*z;
...
void HVector3::RotateY(double angle) {
   double s = sin(angle);
   double c = cos(angle);
   double zz =  z;
   z  = c*zz - s* x;
...
void HVector3::RotateZ(double_t angle) {
   double s = sin(angle);
   double c = cos(angle);
   double xx =  x;
    x = c*xx - s* y;
