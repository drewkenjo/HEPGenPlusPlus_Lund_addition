...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    double big_phi = PHI_VM(ebeamlepton,escatlepton,outPart1,outPart2, 4);   
    double sml_phi = phi_vm(ebeamlepton,escatlepton,outPart1,outPart2,recoilP, 4);
    double cos_the = cos_theta_vm(outPart1,outPart2,recoilP, 4);
...
void HPhysicsGenPHI::addHistograms()
{
    ...
    bookMan->addHBook1D(&weight_sdme,&event->getStruct()->dummyW,100,-0.01,0.15,"weight SDME(Phi)");
    ...
    bookMan->addHBook1D(&thetapi     , &weight,32      , 0,  M_PI,"Theta_k");
    bookMan->addHBook1D(&thetapi     , &weight_sdme, 32, 0,  M_PI,"Theta_k (SDME)");
    bookMan->addHBook1D(&costhetapi  , &weight,100     ,-1,     1,"Cos(Theta_k) hepgen w");
    bookMan->addHBook1D(&costhetapi  , &weight_sdme,100,-1,     1,"Cos(Theta_k) (SDME)");
    bookMan->addHBook1D(&phipi       , &weight,63     ,  0,2*M_PI,"Phi_k");
    bookMan->addHBook1D(&phipi       , &weight_sdme,63,  0,2*M_PI,"Phi_k (SDME)");

    bookMan->addHBook1D(&m_meson,&weight,200,0.8,1.3,"Mass of #phi meson");
...
bool HPhysicsGenPHI::generateMesonMass()
{
    ...
    m_meson = (1.2-2*hepconst::mKaonCharged) * r + 2*hepconst::mKaonCharged;
    double qcms=sqrt(pow((m_meson       /2),2)-hepconst::w2KaonCharged);
    double qphi=sqrt(pow((hepconst::mPhi/2),2)-hepconst::w2KaonCharged);
    double gphi=hepconst::gPhi * pow((qcms/qphi),3) * (hepconst::mPhi/m_meson);
    double prob=m_meson*hepconst::mPhi*gphi*hepconst::gPhi / ( pow((pow(m_meson,2)-pow(hepconst::mPhi,2)),2)+pow(hepconst::mPhi,2)*pow(gphi,2));
    r = myRandom->flat();
    if(r > prob)
    {
        resetcount++;
...
// pi_1 , pi_2, pro_rec as above ...................  
///////////////////////////////////////////////////////////////////////////////////////
double HPhysicsGenPHI::phi_vm( HLorentzVector  LzVecMu0, HLorentzVector LzVecMu1, HLorentzVector LzVecOut1, HLorentzVector  LzVecOut2, HLorentzVector  pr_r, int i4)
{
  ...
  HLorentzVector pr_rvm(  LzVecOut);  pr_rvm.Boost(b); // transform VM to HCMS
  HVector3  pip =  lv1p_vm.Vect();
  HVector3  qq3 =     q_vm.Vect();
  HVector3  VV  =   pr_rvm.Vect();
  double ccos=(( qq3.Cross(VV)).Unit())           .Dot((VV.Cross(pip)).Unit());
  double ssin=(((qq3.Cross(VV)).Cross(VV)).Unit()).Dot((pip.Cross(VV)).Unit());
...
/////////////////////////////////////////////////////////////////////////////
double HPhysicsGenPHI::PHI_VM(HLorentzVector  LzVecMu0, HLorentzVector LzVecMu1, HLorentzVector LzVecOut1, HLorentzVector  LzVecOut2, int i4)
{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
// azym for kaon from phi
// TRENTO CONVENTION .....................
/////////////////////////////
double HPhysicsGenPHI::PHI_PI(HLorentzVector  LzVecMu0, HLorentzVector LzVecMu1, HLorentzVector LzVecOut1, HLorentzVector  LzVecOut2, int i4)
{
  //       LzVecOutX --- pion  
  const double M_Pr = 0.93827203; // proton mass
...
/////////////////////////////////////////////////////////////////////////
double   HPhysicsGenPHI::sdme(double cos_t, double phi_d, double phi_m, int ii){
...
//   ii sign of beam 
  ...
  sd[ 0]= 0.368;  sd[ 1]= 0.018;  sd[ 2]= 0.003;  sd[ 3]=-0.014;
  sd[ 4]=-0.011;  sd[ 5]=-0.032;  sd[ 6]= 0.330;  sd[ 7]= 0.027;
  sd[ 8]=-0.300;  sd[ 9]= 0.017;  sd[10]= 0.080;  sd[11]= 0.186;
  sd[12]=-0.007;  sd[13]=-0.181;  sd[14]= 0.001;  sd[15]=-0.016;
  sd[16]=-0.038;  sd[17]= 0.089;  sd[18]= 0.047;  sd[19]=-0.025;
  sd[20]=-0.067;  sd[21]= 0.059;  sd[22]=-0.041;
...
/////////////////////////////////////////////////////////////////////////////
// cos(\theta) .....................
// pi_1   4 dim table of pion +; pi_2 4 dim table of pion;
//    pro_rec  proton in exit chanel  calculated  from energy  balance 
//    pr_rec_sp=(proton+LzVecMu0)-(LzVecMu1+ LzVecOut1+ LzVecOut2);
//     pro_rec[0]=pr_rec_sp[0] ......    
/////////////////////////////////////////////////////////////////////////////
double  HPhysicsGenPHI::cos_theta_vm(HLorentzVector LzVecOut1, HLorentzVector LzVecOut2, HLorentzVector pr_r,   int i4)
{
  double ss=0. ;
  HVector3 lv,lv1,lv2,b, llv3;
  HLorentzVector LzVecOut ;

  LzVecOut= LzVecOut1+LzVecOut2;
  lv1= LzVecOut1.Vect(); 
  b = - LzVecOut.BoostVector();          // boost vector of pi1+pi2 system
  HLorentzVector lv1p(LzVecOut1); lv1p.Boost(b); // transform p1 4vector to p1+p2 system
  HLorentzVector lvr(pr_r); lvr.Boost(b); // transform proton vector to p1+p2 system
  HVector3 llv1 = lv1p.Vect();
  HVector3 llpr = -lvr.Vect(); // change sign
  ss = cos(llpr.Angle(llv1));
  return ss ; 
} 
